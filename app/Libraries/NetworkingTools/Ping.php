...
class Ping
{
     ...
     */
    public function __construct(string $host, int $ttl = 255)
    {
        if (! isset($host)) {
     ...
     */
    public function ping(string $method = 'exec'): string|int|bool
    {

        return match ($method) {
            'exec' => $this->pingExec(),
            'fsockopen' => $this->pingFsockopen(),
            'socket' => $this->pingSocket(),
            default => false,
        };
    }
     ...
     *
     * @return bool|int|string Latency, in ms.
     *   Latency, in ms.
     */
    private function pingExec(): bool|int|string
    {
        $latency = false;
     ...
     *
     * @return bool|int Latency, in ms.
     *   Latency, in ms.
     */
    private function pingFsockopen(): bool|int
    {
     ...
     *   Latency, in ms.
     */
    private function pingSocket(): bool|int
    {
        // Create a package.
        $type = "\x08";
        $code = "\x00";
